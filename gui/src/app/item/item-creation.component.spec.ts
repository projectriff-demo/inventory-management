import {async, ComponentFixture, TestBed} from '@angular/core/testing';

import {ItemCreationComponent} from './item-creation.component';
import {FormsModule, ReactiveFormsModule} from '@angular/forms';
import {ItemService} from './item.service';
import {Item} from './item';
import {of} from "rxjs";
import {Router} from "@angular/router";

describe('ItemCreationComponent', () => {
  let component: ItemCreationComponent;
  let dom;
  let fixture: ComponentFixture<ItemCreationComponent>;
  let itemServiceSpy: jasmine.SpyObj<ItemService>;
  const routerStub = {
    navigate: jasmine.createSpy('navigate')
  };

  beforeEach(async(() => {
    itemServiceSpy = jasmine.createSpyObj<ItemService>(['save']);
    itemServiceSpy.save.and.returnValue(of({}));
    TestBed.configureTestingModule({
      imports: [
        FormsModule,
        ReactiveFormsModule,
      ],
      declarations: [ItemCreationComponent],
      providers: [
        {provide: ItemService, useValue: itemServiceSpy},
        {provide: Router, useValue: routerStub}
      ],
    }).compileComponents();
  }));

  beforeEach(() => {
    fixture = TestBed.createComponent(ItemCreationComponent);
    component = fixture.componentInstance;
    dom = fixture.debugElement.nativeElement;
    fixture.detectChanges();
  });

  it('should create', () => {
    expect(component).toBeTruthy();
  });

  it('should render a form', () => {
    expect(dom.querySelector('form')).toBeTruthy('form should render');
  });

  describe('when saving an item', () => {
    let form;

    const expectedItem = {
      sku: 'some SKU',
      name: 'some name',
      description: 'some description',
      priceInUsd: 42
    } as Item;

    beforeEach(() => {
      form = dom.querySelector('form');
    });

    it('should call the item creation service on submit', () => {
      setValue(form.querySelector('#sku'), expectedItem.sku);
      setValue(form.querySelector('#name'), expectedItem.name);
      setValue(form.querySelector('#description'), expectedItem.description);
      setValue(form.querySelector('#priceInUsd'), expectedItem.priceInUsd);

      form.querySelector('.button').click();

      expect(itemServiceSpy.save).toHaveBeenCalledWith(expectedItem);
      expect(routerStub.navigate).toHaveBeenCalledWith(['/'], {replaceUrl: true});
    });

    it('should reject an empty SKU', () => {
      setValue(form.querySelector('#name'), expectedItem.name);
      setValue(form.querySelector('#description'), expectedItem.description);
      setValue(form.querySelector('#priceInUsd'), expectedItem.priceInUsd);

      form.querySelector('.button').click();

      expect(itemServiceSpy.save).not.toHaveBeenCalled();
      expect(Array.from(form.querySelector('#sku').classList)).toContain('ng-invalid');
    });

    it('should reject an empty name', () => {
      setValue(form.querySelector('#sku'), expectedItem.sku);
      setValue(form.querySelector('#description'), expectedItem.description);
      setValue(form.querySelector('#priceInUsd'), expectedItem.priceInUsd);

      form.querySelector('.button').click();

      expect(itemServiceSpy.save).not.toHaveBeenCalled();
      expect(Array.from(form.querySelector('#name').classList)).toContain('ng-invalid');
    });

    it('should reject an empty description', () => {
      setValue(form.querySelector('#sku'), expectedItem.sku);
      setValue(form.querySelector('#name'), expectedItem.name);
      setValue(form.querySelector('#priceInUsd'), expectedItem.priceInUsd);

      form.querySelector('.button').click();

      expect(itemServiceSpy.save).not.toHaveBeenCalled();
      expect(Array.from(form.querySelector('#description').classList)).toContain('ng-invalid');
    });

    it('should reject an empty price', () => {
      setValue(form.querySelector('#sku'), expectedItem.sku);
      setValue(form.querySelector('#name'), expectedItem.name);
      setValue(form.querySelector('#description'), expectedItem.description);

      form.querySelector('.button').click();

      expect(itemServiceSpy.save).not.toHaveBeenCalled();
      expect(Array.from(form.querySelector('#priceInUsd').classList)).toContain('ng-invalid');
    });
  });

  const setValue = (element, value) => {
    element.value = value;
    element.dispatchEvent(new Event('input'));
  };
});
